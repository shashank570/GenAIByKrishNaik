...
from langchain.chains import ConversationalRetrievalChain # used instead of RetrievalQA
...
subjects_list = ["Science", "Social Studies", "English"]

# Helper to setup vectorstore and chat_chain
def get_vector_db_path(chapter, subject):
    if chapter == "All Chapters":
        return f"{parent_dir}/vector_db/class_8_{subject.lower()}_vector_db"
...
def setup_chain(selected_chapter, selected_subject):
    ...
    # This stores previous chat messages.
    # Tells memory where the chain stores responses.
    # Conversation history will be stored under: chat_history
    ...
    chain = ConversationalRetrievalChain.from_llm(
        llm=llm,
        memory=memory,
        chain_type="stuff", # stuffs all text into one prompt
        retriever=vectorstore.as_retriever(search_type="mmr", search_kwargs={"k": 3}), # Retrieve top 3 chunks.
        return_source_documents=True,
        get_chat_history=lambda h: h,
        verbose=True
    )
    # MMR = Maximum Marginal Relevance
    # Instead of retrieving similar chunks only, it retrieves: relevant and diverse chunks

    # This is the core RAG pipeline. 
    # It combines: Retriever, Memory, LLM into one conversational QA system 
    # and return chain
...
selected_subject = st.selectbox(
    label="Select a Subject from class 8",
...
if selected_subject:
    ...
    selected_chapter = st.selectbox(
        label=f"Select a Chapter from class 8 - {selected_subject}",
